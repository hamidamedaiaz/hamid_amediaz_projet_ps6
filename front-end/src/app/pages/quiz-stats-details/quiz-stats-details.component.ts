...
@Component({
  ...
  styleUrl: './quiz-stats-details.component.scss'
})
export class QuizStatsDetailsComponent implements OnInit {
  quiz: Quiz | null = null;
  quizId: number = 0;
  ...
  loadQuizData() {
    this.quizService.quizzes$.subscribe(quizzes => {
      this.quiz = quizzes.find(p => p.id === this.quizId) || null;
      if (!this.quiz) {
        this.router.navigate(['/admin']);
      }
    });
  ...
  navigateBack() {
    this.router.navigate(['/admin'], {
      queryParams: { section: 'stats-quiz' }
    });
  }
}
